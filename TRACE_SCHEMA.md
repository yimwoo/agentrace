...
aggregate `time_window`, command working-directory distribution `cwd_counts`, per-working-directory `cwd_totals` with commands run, failure counts, timing totals, status/source distributions, time windows, first/last event references for repeated cwd groups, and linked artifacts, and `first`, `slowest`, `fastest`, and `last` command highlights). JSON reports should also include an `activity_timeline` list that interleaves command timing and file-edit summary rows by available start/end timestamp, preserves source order for untimestamped rows, and carries command identity, cwd, exit status, stdout/stderr previews when available, edit path/kind/line impact, timing source, error context, and linked artifacts for quick chronological inspection. JSON reports should pair that list with `activity_timeline_summary` totals (count, type/status/duration-source distributions, per-type duration totals, per-type `type_duration_share` values derived from total command/edit activity duration, a `dominant_duration_type` highlight with the activity type that consumed the largest share of command/edit activity duration, failed count, aggregate `time_window`, `span_duration_ms` measuring elapsed wall-clock time across the timeline window when both boundaries are available, `covered_duration_ms`, `uncovered_duration_ms`, `uncovered_intervals`, `uncovered_interval_count`, `average_uncovered_interval_ms`, `largest_uncovered_interval`, `coverage_ratio`, `idle_ratio`, `overlap_ratio`, and `covered_interval_count` measuring merged timestamp/duration-backed activity coverage without double-counting overlapping intervals, the remaining observed span not covered by activity intervals, the individual uncovered time windows in the observed span plus their count/average/largest interval highlight, the covered share of the observed span, the idle/uncovered share of the observed span, and the share of the observed span represented by adjacent overlap duration, a `first_activity` highlight for the first command/edit in timeline order, a `slowest_activity` highlight carrying the slowest command/edit identity plus timing, output/error, file-impact, and artifact context, a `fastest_activity` highlight carrying the shortest command...
