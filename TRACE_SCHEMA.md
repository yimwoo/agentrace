...
including unique commands, repeated command retries, per-command attempt totals with attempt-level duration-source counts and time windows, command working-directory counts, failed command identities with timing/failure context, command status counts,
