...
`failed_edits`, edit-kind distribution `kind_counts`, per-kind `kind_totals` with changed files, failure counts, line impact, timing totals, status/source distributions, time windows, first/last event references for repeated edit-kind groups, and linked artifacts, edit `status_counts`, `duration_source_counts`, `duration_recorded_count`, `duration_missing_count`, `duration_coverage_ratio`, aggregate `time_window`,
`net_line_delta`, total/average/average-recorded-only/median edit duration, edit `duration_range_ms`, and `first_edit`, `largest_edit`, `shortest_edit`, and `last_edit` highlights). Failed command and failed edit aggregate rows should retain linked artifact references when available, so report totals can point at command logs or diff artifacts without requiring a detail-row scan. Failed edit
...
command duration source counts, status-duration totals/shares with a dominant status highlight, duration recorded/missing counts and coverage ratio, aggregate command time window, the average, average recorded-only, median, and range of command durations, first, slowest, fastest, and last command identities with
