...
`average_duration_ms`, `average_recorded_duration_ms`, `median_duration_ms`, `failed_count`, `failed_commands`, `exit_code_counts`, `status_counts`, `status_duration_extremes_ms`, `duration_source_counts`, `duration_source_duration_ms`, `duration_source_average_ms`, `duration_source_extremes_ms`, `duration_source_share`,
`duration_recorded_count`, `duration_missing_count`, `duration_coverage_ratio`,
aggregate `time_window`, command working-directory distribution `cwd_counts`, per-working-directory `cwd_totals` with commands run, failure counts, timing totals, status/source distributions, time windows, first/last event references for repeated cwd groups, and linked artifacts, command `duration_range_ms`, command `duration_extremes_ms` min/max bounds, and `first`, `slowest`, `fastest`, and `last` command highlights). JSON reports should also include an `activity_timeline` list that interleaves command timing and file-edit summary rows by available start/end timestamp, preserves source order for untimestamped rows, and carries command identity, cwd, exit status, stdout/stderr previews when available, edit path/kind/line impact, timing source, error context, and linked artifacts for quick chronological inspection. JSON reports should pair that list with `activity_timeline_summary` totals (count, type/status/duration-source distributions, per-status duration totals, average durations, and duration-share values, per-duration-source duration totals, average-duration values, min/max duration extremes, and duration-share values, a `dominant_duration_status` highlight with the status that consumed the largest share of command/edit activity duration, per-type duration totals, per-type `type_duration_share` values derived from total command/edit activity duration, a `dominant_duration_type` highlight with the activity type that consumed the largest share of command/edit activity duration, failed count, aggregate `time_window`, `span_duration_ms` measuring elapsed wall-clock time across the timeline window when both boundaries are available, `covered_duration_ms`, `uncovered_duration_ms`, `uncovered_intervals`, `uncovered_interval_count`, `average_uncovered_interval_ms`, `largest_uncovered_interval`, `coverage_ratio`, `idle_ratio`, `overlap_ratio`, `covered_interval_count`, `merged_covered_interval_count`, and `covered_intervals` listing merged timestamp/duration-backed covered windows, measuring timestamp/duration-backed activity coverage without double-counting overlapping intervals, the raw covered activity interval count, the merged covered-window count, the remaining observed span not covered by activity intervals, the individual covered and uncovered time windows in the observed span plus uncovered count/average/largest interval highlight, the covered share of the observed span, the idle/uncovered share of the observed span, and the share of the observed span represented by adjacent overlap duration, a `first_activity` highlight for the first command/edit in timeline order, a `slowest_activity` highlight carrying the slowest command/edit identity plus timing, output/error, file-impact, and artifact context, a `fastest_activity` highlight carrying the shortest command...
...
`failed_edits`, edit-kind distribution `kind_counts`, per-kind `kind_totals` with changed files, failure counts, line impact, timing totals, status/source distributions, time windows, first/last event references for repeated edit-kind groups, and linked artifacts, edit `status_counts`, `status_duration_extremes_ms`, `duration_source_counts`, `duration_source_duration_ms`, `duration_source_average_ms`, `duration_source_extremes_ms`, `duration_source_share`, `duration_recorded_count`, `duration_missing_count`, `duration_coverage_ratio`, aggregate `time_window`,
`net_line_delta`, total/average/average-recorded-only/median edit duration, edit `duration_range_ms`, edit `duration_extremes_ms` min/max bounds, and `first_edit`, `largest_edit`, `shortest_edit`, and `last_edit` highlights). Failed command and failed edit aggregate rows should retain linked artifact references when available, so report totals can point at command logs or diff artifacts without requiring a detail-row scan. Failed edit
...
repeated command retries, per-command attempt totals with attempt-level duration-source counts, time windows, and artifacts, command working-directory counts and per-cwd timing totals with artifacts, failed command identities with timing/failure context, command exit-code distributions, command status counts,
command duration source counts plus duration totals/averages/shares by source, status-duration totals/averages/min-max extremes/shares with a dominant status highlight, duration recorded/missing counts and coverage ratio, aggregate command time window, the average, average recorded-only, median, range, and min/max extremes of command durations, first, slowest, fastest, and last command identities with
their timing context, changed-file list, per-file change totals with file-level duration-source counts, time windows, and artifacts, net line delta, edit failure counts/status
distribution, edit status-duration totals/averages/min-max extremes/shares with a dominant status highlight, edit-kind counts and per-kind timing/line-impact totals with artifacts, failed edit identities with timing/error context, edit duration source counts plus duration totals/averages/shares by source, edit duration recorded/missing counts and coverage ratio, aggregate edit time window, average
edit duration, average recorded-only edit duration, median edit duration, edit duration range, edit duration min/max extremes, first, largest, shortest, and last edits with their timing context when present, and a compact activity timeline summary with type/status/source distributions, duration recorded/missing counts and coverage ratio, per-type, per-status, and per-duration-source duration totals, status-duration averages and min/max extremes, source-duration averages, and duration shares, the dominant duration type/share, dominant duration status/share, failure count, first activity, slowest activity, fastest activity, last activity, idle gap totals/average/largest gap, overlap totals/average/largest overlap and overlap ratio, first failed and all failed activity identities, span duration, covered duration, covered interval windows, raw and merged covered interval counts, uncovered duration, uncovered interval windows plus uncovered count/average/largest interval, coverage, idle, and overlap ratios, total/average/average-recorded-only/median duration, duration range, duration min/max extremes, and time window. Markdown reports should also render an `Activity Timeline` section after the detailed command and edit sections, interleaving commands and edits in chronological order with the same timing, status, command output-preview, file-impact, error, and artifact context.
