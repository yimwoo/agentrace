...
`duration_recorded_count`, `duration_missing_count`, `duration_coverage_ratio`, `summary_recorded_count`, `summary_missing_count`, `summary_coverage_ratio`, `summary_examples`, `summary_missing_examples`, grouped command `cwd_summary_examples`/`cwd_summary_missing_examples` and `status_summary_examples`/`status_summary_missing_examples` for representative explained and unexplained commands by working directory and status,
...
command duration source counts plus duration totals/averages/shares by source, exit-code duration totals/averages/min-max extremes/coverage/shares with a dominant exit-code duration highlight, exit-code summary coverage, compact exit-code summary/missing-summary examples, grouped command cwd/status summary and missing-summary examples, status-duration totals/averages/min-max extremes/coverage/shares with a dominant status highlight, duration recorded/missing counts and coverage ratio, command summary recorded/missing counts and coverage ratio plus compact command summary examples, aggregate command time window, the average, average recorded-only, median, range, and min/max extremes of command durations, first, slowest, fastest, and last command identities with
