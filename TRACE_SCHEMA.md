...
aggregate `time_window`, command working-directory distribution `cwd_counts`, per-working-directory `cwd_totals` with commands run, failure counts, timing totals, status/source distributions, time windows, first/last event references for repeated cwd groups, and linked artifacts, and `first`, `slowest`, `fastest`, and `last` command highlights). JSON reports should also include an `activity_timeline` list that interleaves command timing and file-edit summary rows by available start/end timestamp, preserves source order for untimestamped rows, and carries command identity, cwd, exit status, stdout/stderr previews when available, edit path/kind/line impact, timing source, error context, and linked artifacts for quick chronological inspection. JSON reports should pair that list with `activity_timeline_summary` totals (count, type/status/duration-source distributions, per-type duration totals, a `dominant_duration_type` highlight with the activity type that consumed the largest share of command/edit activity duration, failed count, aggregate `time_window`, `span_duration_ms` measuring elapsed wall-clock time across the timeline window when both boundaries are available, `covered_duration_ms`, `uncovered_duration_ms`, `uncovered_intervals`, `coverage_ratio`, `idle_ratio`, `overlap_ratio`, and `covered_interval_count` measuring merged timestamp/duration-backed activity coverage without double-counting overlapping intervals, the remaining observed span not covered by activity intervals, the individual uncovered time windows in the observed span, the covered share of the observed span, the idle/uncovered share of the observed span, and the share of the observed span represented by adjacent overlap duration, a `first_activity` highlight for the first command/edit in timeline order, a `slowest_activity` highlight carrying the slowest command/edit identity plus timing, output/error, file-impact, and artifact context, a `fastest_activity` highlight carrying the shortest command/edit identity plus the same timing/output/error/file-impact/artifact context, a `last_activity` highlight for the final command/edit in timeline order, `inter_activity_gaps` with measured idle gaps between adjacent timestamped timeline rows, `total_idle_gap_ms`, `largest_idle_gap`, `inter_activity_overlaps` with measured overlaps when the next activity starts before the previous activity ends, `total_overlap_ms`, `largest_overlap`, `first_failed_activity`, and `failed_activity` rows with command/edit identity plus timing, output/error, file-impact, and artifact context). JSON reports should also include
...
command duration source counts, aggregate command time window, the average and median command durations, first, slowest, fastest, and last command identities with
...
edit duration, median edit duration, first, largest, shortest, and last edits with their timing context when present, and a compact activity timeline summary with type/status/source distributions, per-type duration totals, the dominant duration type/share, failure count, first activity, slowest activity, fastest activity, last activity, idle gap totals/average/largest gap, overlap totals/average/largest overlap and overlap ratio, first failed and all failed activity identities, span duration, covered and uncovered duration/count of timestamp-backed intervals, uncovered interval windows, coverage, idle, and overlap ratios, total/average/median duration, and time window. Markdown reports should also render an `Activity Timeline` section after the detailed command and edit sections, interleaving commands and edits in chronological order with the same timing, status, command output-preview, file-impact, error, and artifact context.
