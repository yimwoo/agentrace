...
command duration source counts plus duration totals/averages/shares by source, status-duration totals/averages/min-max extremes/coverage/shares with a dominant status highlight, duration recorded/missing counts and coverage ratio, command summary recorded/missing counts and coverage ratio plus compact command summary examples, aggregate command time window, the average, average recorded-only, median, range, and min/max extremes of command durations, first, slowest, fastest, and last command identities with
