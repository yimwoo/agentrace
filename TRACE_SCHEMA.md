...
failure counts, status distribution, duration source distribution, repeated-group median/range/min-max duration metrics, duration recorded/missing counts, duration coverage ratio, duration-source duration totals/shares, aggregate time window, first/last event references, and linked artifacts, `total_duration_ms`,
...
aggregate `time_window`, command working-directory distribution `cwd_counts`, per-working-directory `cwd_totals` with commands run, failure counts, timing totals, repeated-group median/range/min-max duration metrics, duration recorded/missing counts, duration coverage ratio, duration-source duration totals/shares, status/source distributions, time windows, first/last event references for repeated cwd groups, and linked artifacts, command `duration_range_ms`, command `duration_extremes_ms` min/max bounds, and `first`, `slowest`, `fastest`, and `last` command highlights). JSON reports should also include an `activity_timeline` list that interleaves command timing and file-edit summary rows by available start/end timestamp, preserves source order for untimestamped rows, and carries command identity, cwd, exit status, stdout/stderr previews when available, edit path/kind/line impact, timing source, error context, and linked artifacts for quick chronological inspection.
JSON reports should pair that list with `activity_timeline_summary` totals (count, type/status/duration-source distributions, per-status duration totals, average durations, and duration-share values, per-duration-source duration totals, average-duration values, min/max duration extremes, and duration-share values, a `dominant_duration_status` highlight, per-type duration totals, `type_average_duration_ms`, `type_duration_extremes_ms` min/max bounds, per-type `type_duration_share` values, a `dominant_duration_type` highlight, failed count, aggregate `time_window`, `span_duration_ms`, covered/uncovered interval metrics, coverage/idle/overlap ratios, first/slowest/fastest/last activity highlights, failed-activity identities, idle gap totals, and overlap totals).
`edit_summary_totals` (`count`, deduplicated changed files, per-file `file_change_totals` with edit counts,
failure counts, total added/removed/net lines, total/average duration, median duration, duration range, min/max duration extremes for repeated file groups, duration recorded/missing counts, duration coverage ratio, duration-source duration totals/shares, status distribution, kind distribution, duration source distribution, aggregate time window, first/last event references for repeated file groups, and linked artifacts, total added/removed lines, edit `failed_count`,
`failed_edits`, edit-kind distribution `kind_counts`, per-kind `kind_totals` with changed files, failure counts, line impact, timing totals, average/median/range/min-max duration metrics for repeated kind groups, duration recorded/missing counts, duration coverage ratio, duration-source duration totals/shares, status/source distributions, time windows, first/last event references for repeated edit-kind groups, and linked artifacts, edit `status_counts`, `status_duration_extremes_ms`, `duration_source_counts`, `duration_source_duration_ms`, `duration_source_average_ms`, `duration_source_extremes_ms`, `duration_source_share`, `duration_recorded_count`, `duration_missing_count`, `duration_coverage_ratio`, aggregate `time_window`,
...
repeated command retries, per-command attempt totals with attempt-level duration-source counts/totals/shares, time windows, and artifacts, command working-directory counts and per-cwd timing totals with duration-source counts/totals/shares and artifacts, failed command identities with timing/failure context, command exit-code distributions, command status counts,
