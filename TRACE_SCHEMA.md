...
`net_line_delta`, total/average/median edit duration, and `largest_edit`). Failed command and failed edit aggregate rows should retain linked artifact references when available, so report totals can point at command logs or diff artifacts without requiring a detail-row scan. Failed edit
...
command duration source counts, aggregate command time window, the average and median command durations, slowest and fastest command identities with
