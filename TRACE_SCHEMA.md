...
repeated command retries, per-command attempt totals with attempt-level duration-source counts/totals/shares, time windows, and artifacts, command working-directory counts, cwd duration totals/averages/min-max extremes/coverage/shares and dominant cwd highlights, per-cwd timing totals with duration-source counts/totals/shares and artifacts, failed command identities with timing/failure context, command exit-code distributions, command status counts,
command duration source counts plus duration totals/averages/shares by source, status-duration totals/averages/min-max extremes/coverage/shares with a dominant status highlight, duration recorded/missing counts and coverage ratio, command summary recorded/missing counts and coverage ratio, aggregate command time window, the average, average recorded-only, median, range, and min/max extremes of command durations, first, slowest, fastest, and last command identities with
their timing context and command summaries when present, changed-file list, per-file change totals with file-level duration-source counts/totals/shares, time windows, and artifacts, net line delta, edit failure counts/status
distribution, edit status-duration totals/averages/min-max extremes/coverage/shares with a dominant status highlight, edit-kind counts, edit-kind duration totals/averages/min-max extremes/coverage/shares and dominant kind highlights, per-kind timing/line-impact totals with duration-source counts/totals/shares and artifacts, failed edit identities with timing/error context, edit duration source counts plus duration totals/averages/shares by source, edit duration recorded/missing counts and coverage ratio, edit summary recorded/missing counts and coverage ratio, aggregate edit time window, average
edit duration, average recorded-only edit duration, median edit duration, edit duration range, edit duration min/max extremes, first, largest, slowest, shortest, and last edits with their timing context and edit summaries when present, and a compact activity timeline summary with type/status/source distributions, duration recorded/missing counts and coverage ratio, summary recorded/missing counts and coverage ratio, per-type, per-status, and per-duration-source duration totals, type-duration averages, min/max extremes, and coverage, status-duration averages, min/max extremes, and coverage, source-duration averages, and duration shares, the dominant duration type/share, dominant duration status/share, failure count, first activity, slowest activity, fastest activity, last activity, idle gap totals/average/largest gap, overlap totals/average/largest overlap and overlap ratio, first failed and all failed activity identities, span duration, covered duration, covered interval windows, raw and merged covered interval counts, uncovered duration, uncovered interval windows plus uncovered count/average/largest interval, coverage, idle, and overlap ratios, total/average/average-recorded-only/median duration, duration range, duration min/max extremes, and time window. Markdown reports should also render a compact `report_summary_coverage` line near the top-level totals, including per-command and per-file coverage labels, plus an `Activity Timeline` section after the detailed command and edit sections, interleaving commands and edits in chronological order with the same timing, status, command output-preview, file-impact, error, and artifact context.
