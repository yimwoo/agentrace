...
`failed_edits`, edit-kind distribution `kind_counts`, per-kind `kind_totals` with changed files, failure counts, line impact, timing totals, status/source distributions, time windows, first/last event references for repeated edit-kind groups, and linked artifacts, edit `status_counts`, `duration_source_counts`, `duration_source_duration_ms`, `duration_source_average_ms`, `duration_source_share`, `duration_recorded_count`, `duration_missing_count`, `duration_coverage_ratio`, aggregate `time_window`,
...
command duration source counts plus duration totals/averages/shares by source, status-duration totals/shares with a dominant status highlight, duration recorded/missing counts and coverage ratio, aggregate command time window, the average, average recorded-only, median, range, and min/max extremes of command durations, first, slowest, fastest, and last command identities with
