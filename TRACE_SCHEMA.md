...
Markdown reports should also render `report_summary_text_metrics` near the top-level summary, mirroring the JSON command/edit/activity summary text character counts (`summary_text_count`, `summary_text_total_chars`, `summary_text_average_chars`, `summary_text_min_chars`, `summary_text_max_chars`, `summary_text_empty_count`, `summary_text_coverage_ratio`, and `summary_text_missing_ratio`) so reviewers can quickly tell whether command and edit summaries are absent, terse, content-rich, or only partially covered before scanning detail rows.
