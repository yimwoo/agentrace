# T-122: Add command timing and edit summaries to reports
...
- Added report summary-duration-impact spread fields for rows without human-readable summaries.
- JSON `report_summary_duration_impact` buckets now include `summary_missing_duration_range_ms` and `summary_missing_duration_extremes_ms` for command, edit, and combined activity rows.
- Markdown renders the new missing-duration spread fields in the top-level `report_summary_duration_impact` line.
