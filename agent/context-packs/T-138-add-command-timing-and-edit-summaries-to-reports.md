# T-138: Add command timing and edit summaries to reports
...
- JSON `report_summary_duration_impact` command/edit/activity buckets include `summary_missing_duration_concentration` with `none`, `single_row`, `clustered`, or `distributed` values.
- Markdown renders `missing_duration_concentration` in the top-level `report_summary_duration_impact` line.
- Regression coverage exercises clustered and distributed missing-duration concentration labels.
