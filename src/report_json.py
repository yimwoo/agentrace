...
def _summary_text_coverage_duration_share_gap_label(abs_ratio):
    """Label normalized summary-coverage versus summarized-duration-share skew."""
    if abs_ratio >= 0.75:
        return "high_coverage_duration_share_gap"
    if abs_ratio >= 0.25:
        return "medium_coverage_duration_share_gap"
    if abs_ratio > 0:
        return "low_coverage_duration_share_gap"
...
def _summary_text_coverage_duration_share_gap_attention_label(abs_ratio):
    """Return review-attention label for coverage versus duration-share skew."""
    if abs_ratio >= 0.75:
        return "high_coverage_duration_share_gap_attention"
    if abs_ratio >= 0.25:
        return "medium_coverage_duration_share_gap_attention"
    if abs_ratio > 0:
        return "low_coverage_duration_share_gap_attention"
...
def _summary_text_coverage_duration_share_gap_attention_status(abs_ratio):
    """Return whether coverage versus duration-share skew needs review attention."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_attention_needed"
    return "no_coverage_duration_share_gap_attention_needed"


def _summary_text_coverage_duration_share_gap_attention_action(abs_ratio):
    """Return the suggested review action for coverage versus duration-share skew."""
    if abs_ratio > 0:
        return "review_coverage_duration_share_gap"
    return "no_coverage_duration_share_gap_action"


def _summary_text_coverage_duration_share_gap_attention_action_priority_label(abs_ratio):
    """Return a review-priority label for the coverage versus duration-share skew action."""
    if abs_ratio >= 0.75:
        return "high_priority_coverage_duration_share_gap_attention_action"
    if abs_ratio >= 0.25:
        return "medium_priority_coverage_duration_share_gap_attention_action"
    if abs_ratio > 0:
        return "low_priority_coverage_duration_share_gap_attention_action"
    return "no_priority_coverage_duration_share_gap_attention_action"


def _summary_text_coverage_duration_share_gap_attention_action_priority_rank(abs_ratio):
    """Return sortable review-priority rank for the coverage versus duration-share skew action."""
    if abs_ratio >= 0.75:
        return 3
    if abs_ratio >= 0.25:
        return 2
...
def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_rank(abs_ratio):
    """Return sortable binary rank for the coverage-duration-share status reason."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail(abs_ratio):
    """Return an explicit detail phrase for the coverage-duration-share status reason."""
    if abs_ratio > 0:
        return "priority_status_active_because_coverage_duration_share_gap_present"
    return "priority_status_inactive_because_coverage_duration_share_gap_absent"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_rank(abs_ratio):
    """Return sortable binary rank for the explicit coverage-duration-share reason detail."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_status(abs_ratio):
    """Return an explicit active/inactive status for the coverage-duration-share reason detail."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active"
    return "coverage_duration_share_gap_priority_reason_detail_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active(abs_ratio):
    """Return a machine-friendly boolean for active coverage-duration-share reason details."""
    return abs_ratio > 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_rank(abs_ratio):
    """Return sortable binary rank for active coverage-duration-share reason details."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status(abs_ratio):
    """Return active/inactive status for the coverage-duration-share reason detail active flag."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active"
    return "coverage_duration_share_gap_priority_reason_detail_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank(abs_ratio):
    """Return sortable binary rank for active coverage-duration-share reason detail statuses."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status(abs_ratio):
    """Return active/inactive status for the active reason detail status rank."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank(abs_ratio):
    """Return sortable binary rank for active reason detail status-rank statuses."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status(abs_ratio):
    """Return active/inactive status for the active reason detail status-rank status rank."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank(abs_ratio):
    """Return sortable binary rank for active reason detail status-rank status-rank statuses."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status(abs_ratio):
    """Return active/inactive status for the active reason detail status-rank status-rank status rank."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank(abs_ratio):
    """Return sortable binary rank for active reason detail status-rank status-rank status-rank statuses."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status(abs_ratio):
    """Return active/inactive status for active reason detail status-rank status-rank status-rank status ranks."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank(abs_ratio):
    """Return sortable binary rank for active reason detail status-rank status-rank status-rank status-rank statuses."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status(abs_ratio):
    """Return active/inactive status for active reason detail status-rank status-rank status-rank status-rank rank values."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(abs_ratio):
    """Return sortable binary rank for active reason detail status-rank status-rank status-rank status-rank rank statuses."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(abs_ratio):
    """Return active/inactive status for active reason detail status-rank status-rank status-rank status-rank status-rank values."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(abs_ratio):
    """Return sortable binary rank for active reason detail status-rank status-rank status-rank status-rank status-rank statuses."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(abs_ratio):
    """Return active/inactive status for the active reason detail status-rank status-rank status-rank status-rank status-rank status rank."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(abs_ratio):
    """Return sortable binary rank for active reason detail status-rank status-rank status-rank status-rank status-rank status-rank statuses."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(abs_ratio):
    """Return active/inactive status for active reason detail status-rank status-rank status-rank status-rank status-rank status-rank status ranks."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(abs_ratio):
    """Return sortable binary rank for active reason detail status-rank status-rank status-rank status-rank status-rank status-rank status-rank statuses."""
    if abs_ratio > 0:
        return 1
    return 0
...
def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(abs_ratio):
    """Return active/inactive status for active reason detail status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank values."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(abs_ratio):
    """Return sortable binary rank for active reason detail status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank status values."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(abs_ratio):
    """Return active/inactive status for active reason detail status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank status rank."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(abs_ratio):
    """Return sortable binary rank for active reason detail status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank values."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(abs_ratio):
    """Return active/inactive status for active reason detail status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank rank values."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(abs_ratio):
    """Return sortable binary rank for active reason detail status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank rank statuses."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(abs_ratio):
    """Return active/inactive status for active reason detail status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank rank values."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_inactive"


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(abs_ratio):
    """Return sortable binary rank for active reason detail status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank rank statuses."""
    if abs_ratio > 0:
        return 1
    return 0


def _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(abs_ratio):
    """Return active/inactive status for active reason detail status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank status-rank rank values."""
    if abs_ratio > 0:
        return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_active"
    return "coverage_duration_share_gap_priority_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_inactive"
...
def _summary_text_coverage_duration_share_gap_direction(delta):
    ...
    if delta > 0:
        return "coverage_overrepresents_duration_share"
    if delta < 0:
        return "coverage_underrepresents_duration_share"
    return "balanced_coverage_duration_share"
...
def _summary_text_metrics(rows):
    ...
    summarized_duration_ms = sum(
        _numeric_value(row.get("duration_ms")) for row in normalized_rows if row.get("summary")
    )
    ...
    return {
        ...
        "summary_text_coverage_duration_share_gap_direction": _summary_text_coverage_duration_share_gap_direction(
            coverage_duration_share_delta,
        ),
        ...
        "summary_text_coverage_duration_share_gap_attention_status": (
            _summary_text_coverage_duration_share_gap_attention_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action": (
            _summary_text_coverage_duration_share_gap_attention_action(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_label": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_label(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        ...
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        "summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status": (
            _summary_text_coverage_duration_share_gap_attention_action_priority_status_reason_detail_active_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status_rank_status(
                coverage_duration_share_delta_abs_ratio,
            )
        ),
        ...
        "summary_text_summarized_duration_ms_per_char": (
            0 if not total_chars else round(summarized_duration_ms / total_chars, 4)
        ),
...
def _summary_timing_window_gap_delta_comparison_attention_rank(duration_minus_count_delta_abs):
    """Return a sortable severity rank for missing-window gap signal divergence."""
    if duration_minus_count_delta_abs <= 0:
        return 0
    if duration_minus_count_delta_abs >= 0.5:
        return 3
...
def _summary_timing_window_gap_delta_comparison_attention_status(duration_minus_count_delta_abs):
    """Return a compact status for missing-window gap signal divergence attention."""
    if duration_minus_count_delta_abs > 0:
        return "missing_summary_gap_signal_attention_needed"
    return "no_missing_summary_gap_signal_attention_needed"


def _summary_timing_window_gap_delta_comparison_attention_trigger(duration_minus_count_delta):
    """Name which sparse-summary signal drives count-vs-duration gap attention."""
    if duration_minus_count_delta > 0:
        return "duration_share_missing_summary_gap_signal_attention_trigger"
    if duration_minus_count_delta < 0:
        return "count_share_missing_summary_gap_signal_attention_trigger"
    return "no_missing_summary_gap_signal_attention_trigger"


def _summary_timing_window_gap_delta_comparison_attention_trigger_signal(duration_minus_count_delta):
    """Return the compact signal family behind gap-comparison attention."""
    if duration_minus_count_delta > 0:
        return "duration_share"
    if duration_minus_count_delta < 0:
        return "count_share"
    return "none"
...
def _summary_timing_window_gap_delta_comparison_attention_trigger_signal_status(duration_minus_count_delta):
    """Return a compact status for whether the trigger signal needs attention."""
    if duration_minus_count_delta:
        return "missing_summary_gap_trigger_signal_attention_needed"
...
def _summary_timing_window_gap_delta_comparison_attention_trigger_signal_action_label(duration_minus_count_delta):
    """Return a readable action label for the compact gap-comparison trigger signal."""
    if duration_minus_count_delta > 0:
        return "duration_share_missing_summary_gap_signal_review_action"
    if duration_minus_count_delta < 0:
        return "count_share_missing_summary_gap_signal_review_action"
    return "no_missing_summary_gap_trigger_signal_review_action"


def _summary_timing_window_gap_delta_comparison_attention_trigger_signal_action_priority_label(duration_minus_count_delta_abs):
    """Return a review-priority label for the compact gap-comparison trigger action."""
    if duration_minus_count_delta_abs >= 0.5:
        return "high_priority_missing_summary_gap_trigger_signal_review_action"
    if duration_minus_count_delta_abs >= 0.25:
        return "medium_priority_missing_summary_gap_trigger_signal_review_action"
    if duration_minus_count_delta_abs > 0:
        return "low_priority_missing_summary_gap_trigger_signal_review_action"
    return "no_priority_missing_summary_gap_trigger_signal_review_action"


def _summary_timing_window_gap_delta_comparison_attention_trigger_signal_action_priority_rank(duration_minus_count_delta_abs):
    """Return a sortable review-priority rank for the compact gap-comparison trigger action."""
    if duration_minus_count_delta_abs >= 0.5:
        return 3
    if duration_minus_count_delta_abs >= 0.25:
        return 2
...
def _summary_complete_window_duration_delta_label(duration_delta_ms):
    """Label the signed complete-window duration delta by summary bucket."""
    if duration_delta_ms > 0:
        return "missing_summary_complete_window_duration_higher"
    if duration_delta_ms < 0:
        return "recorded_summary_complete_window_duration_higher"
...
def _summary_timing_window_metrics(rows):
    ...
    missing_window_gap_delta_max = max(
        0,
        missing_window_share_delta,
        missing_window_duration_share_delta,
    )
    summary_missing_window_gap_duration_minus_count_delta = round(
    ...
    summary_complete_window_duration_delta_abs_ms = abs(summary_complete_window_duration_delta_ms)
    summary_complete_window_duration_delta_abs_share = (
        0 if not complete_window_duration_total_ms else round(
            summary_complete_window_duration_delta_abs_ms / complete_window_duration_total_ms,
            4,
        )
    )
    ...
    summary_complete_window_duration_ratio = (
        None if not summarized_complete_duration_ms and unsummarized_complete_duration_ms else (
            0 if not summarized_complete_duration_ms else round(
                unsummarized_complete_duration_ms / summarized_complete_duration_ms,
                4,
            )
        )
    ...
    return {
        ...
        "summary_complete_window_duration_delta_label": _summary_complete_window_duration_delta_label(
            summary_complete_window_duration_delta_ms,
        ),
        ...
        "summary_missing_window_gap_delta_comparison_attention_rank": (
            _summary_timing_window_gap_delta_comparison_attention_rank(
                summary_missing_window_gap_duration_minus_count_delta_abs,
            )
        ),
        ...
        "summary_missing_window_gap_delta_comparison_attention_status": (
            _summary_timing_window_gap_delta_comparison_attention_status(
                summary_missing_window_gap_duration_minus_count_delta_abs,
            )
        ),
        "summary_missing_window_gap_delta_comparison_attention_trigger": (
            _summary_timing_window_gap_delta_comparison_attention_trigger(
                summary_missing_window_gap_duration_minus_count_delta,
            )
        ),
        "summary_missing_window_gap_delta_comparison_attention_trigger_signal": (
        ...
        "summary_missing_window_gap_delta_comparison_attention_trigger_signal_action_label": (
            _summary_timing_window_gap_delta_comparison_attention_trigger_signal_action_label(
                summary_missing_window_gap_duration_minus_count_delta,
            )
        ),
        "summary_missing_window_gap_delta_comparison_attention_trigger_signal_action_priority_label": (
            _summary_timing_window_gap_delta_comparison_attention_trigger_signal_action_priority_label(
                summary_missing_window_gap_duration_minus_count_delta_abs,
            )
        ),
        "summary_missing_window_gap_delta_comparison_attention_trigger_signal_action_priority_rank": (
            _summary_timing_window_gap_delta_comparison_attention_trigger_signal_action_priority_rank(
                summary_missing_window_gap_duration_minus_count_delta_abs,
            )
        ),
