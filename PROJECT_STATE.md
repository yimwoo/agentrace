...
- top-level JSON and Markdown reports now include `report_summary_duration_impact`, which totals summarized vs. unsummarized command, edit, and combined activity duration and names the highest-duration missing-summary examples for each bucket to highlight sparse high-duration rows before detail scanning
