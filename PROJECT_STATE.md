...
- top-level JSON and Markdown reports now include `report_summary_duration_impact`, which totals summarized vs. unsummarized command, edit, and combined activity duration, includes recorded/missing/total row counts, average, median, range, min/max extremes, duration-source counts, and highest-duration compact examples for summarized row durations, plus average, median, range, min/max extremes, and duration-source counts for unsummarized row durations in each bucket, surfaces the largest missing-summary duration and its share of missing-summary duration, and names the highest-duration missing-summary examples to highlight whether sparse summary coverage is concentrated in one expensive row or spread across many rows before detail scanning
