...
- JSON and Markdown reports now surface command timing rows and file edit summaries with per-row net line deltas (including compact run-summary rows), linked command-log/diff artifact references, row duration sources, and available start/end timestamp context; summary-only fallback rows are normalized with derived durations/duration sources and computed edit net line deltas when older summaries omit those fields
